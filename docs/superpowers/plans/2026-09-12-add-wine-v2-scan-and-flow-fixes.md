...
21. **Owner approvals after L1 (2026-09-13).** The owner answered L1b's owner items with "1 yes, 2 yes, 3 yes, 4 default, 5 default". Items 4 and 5 are blind-tasting defaults (Q7, Q8), recorded in that ledger. Add-wine ledger D18. This supersedes L1's task text, spec §A.3's field descriptions and the old B.5 step 8 / B.7 / E.2 rules where they differ; the spec now states the new rules (§B.5 step 8, §B.7, §E.2, §F.3 Q22).
    - **Reader instructions (approval 1, done).** Commit 11d4c3f, under L1's OWNS clause for `label-read-schema.ts` (spec §F.3 Q22), with tests in `label-read-schema.test.ts`. Only field descriptions change: `LABEL_READ_PROMPT` is unchanged, and the schema JSON grows by 607 characters (about 150 input tokens per scan).
      - `noGeographicIndication` is true only for the table-wine categories themselves (Vin de France, Vino d'Italia, Deutscher Wein, Vino de España, a plain table wine). It is false whenever the label names a place of origin below country level ("Mendoza"), with or without a designation term, and false when the appellation could not be read (#7, El Enemigo).
      - `designation`'s examples are active `type_designations` names: "Grosses Gewächs (GG)", "Vintage Port", "Late Bottled Vintage (LBV)". "Gran Selezione" and "VORS" have no row and were dropped; no example was added.
      - `producer` is the name the label presents as its brand ("J.M. Boillot"), with a title word that is part of the name. A bottler or company line ("Mis en bouteille par…", "Produced and bottled by…") is used only when no other name is printed (#2).
      - `appellation`: when the label prints an official regional origin without a designation term (a Chinese 产区 such as "Ningxia", an Argentine IG), the reader returns that origin's name (#4, Changyu).
      - `region` is null when only a brand and a grape are printed and the origin is not certain (#15, Tridente).
    - **Designation short forms (approval 1b, done; spec §B.5 step 8).** Also 11d4c3f, in `resolve.ts` with tests in `resolve.test.ts`. The rule runs only when folded equality picks nothing:
      - a row whose name ends in exactly one bracket matches when the bracket's content ("LBV") or the name without the bracket ("Grosses Gewächs") folds equal to the read;
      - candidates are the draft country's rows and country-less rows; another country's row is never a candidate;
      - exactly one candidate is a match. Two or more pick nothing, a country row and a country-less row included, so "Vintage" alone matches neither "Vintage Port" nor "Late Bottled Vintage (LBV)";
      - no pinned `live-replay.test.ts` outcome changes.
    - **Lookup order (approval 3's lookup half; committed, live apply pending).** Commit aa54499 adds `20260914112500_producer_lookup_exact_then_wines.sql`, dry-run only. The file is in no task's OWNS. The main session applies it live, repeating amendment 11's version check first.
      - `find_producer_by_folded_name` keeps its signature, SECURITY INVOKER, `search_path` and grants. Only the ORDER BY changes: the given region (null-safe since 20260912101530), then the exact spelling (`lower(p.name) = lower(btrim(p_name))`), then a producer that holds wines (a `catalog_wines` or `wine_answers` row carries its id, read under the caller's RLS), then any region link, then name and id (spec §B.7, §E.2).
      - It adds `wine_answers_producer_id_idx` and runs `analyze public.producers`, so the wines key probes per candidate instead of scanning every answer row.
      - `find_or_create_producer` takes the order at run time and is asserted unchanged.
      - `__fixtures__/snapshot-lookup.ts` mirrors the order (optional `has_wines`; absent means false), and `producer-lookup-order.test.ts` pins it.
      - **Pending:** `live-replay.test.ts` #13 (Vidal-Fleury) has failed since aa54499 (202 of 203 tests). Under the new order the read lands on `58116bac` "Vidal-Fleury", not on the pinned twin `9f9c976f`. A re-pin patch exists in the main session's scratchpad (`lookup-fix/live-replay-13-repin.patch`); L1, which owns that file, applies it. `npm test` is not green until then.
    - **Catalog corrections and producer merges (approvals 2 and 3; main-session data change, live run pending).** One transaction in the gitignored `.superpowers/add-wine-v2/l1/catalog-fixes.mjs`, no migration. Its `--mode dry` run rolled back and was verified; the main session runs `--mode live`. The run and the lookup migration can go live in either order. It first asserts every id and current value, and fails closed on any drift, an unreviewed trigger or a unique collision.
      - #1 Château Saint Georges 2012 (`b7ca3f8a`): appellation Saint-Émilion AOC (`151dc287`) → Saint-Émilion Grand Cru AOC (`814f0f40`).
      - #3 Paitin Barbaresco Riserva 2015 (`8649be76`): producer Vecchie Vigne Paitin (`38a85744`) → Paitin (`c5b25ba2`).
      - #5 Rosso della Badia NV (`9a4af629`): producer Gasleni Alberti (`6b90fa8b`) → Badia di Morrona (`22c8b23c`); primary grape "Sangiovese blend" (`00456ae7`) → Sangiovese (`54e7ab7d`); wine name "Badia di Morrona Rosso della Badia" → "Rosso della Badia". The grape is written to the wine and to its `catalog_wine_grapes` row (`937184eb`) together, because the `catalog_wine_grapes_recompute` trigger rebuilds `primary_grape_id` from that table.
      - #6 Ampelos 2021 (`8344ecc1`): producer Ampelos Vineyard (`795ca7d6`) → Ampelos (`a2e0f8f0`); appellation California AVA (`ef4ebc71`) → Sta. Rita Hills AVA (`5e60cebb`).
      - #12 Borges Vintage Port 2004 (`86253e5a`): its producer row `6aaef358` is renamed in place, "Borges (Sociedade dos Vinhos Borges, S.A.)" → "Sociedade dos Vinhos Borges". The wine row itself is not written.
      - #15 Tridente Tempranillo 2018 (`c4e1429e`): region Castilla La Mancha (`528b5672`) → Castilla y Leon (`eecc95bf`); appellation Castilla La Mancha (`d17226e0`) → Castilla y Leon (`5dd53734`); producer Tridente (`0d1d099c`) → Bodegas Tridente (`7f46bd24`).
      - **Merges** into the copy that holds the wines: "J. M. Boillot" (`745fc108`) and "J. M.Boillot" (`b50dfcdf`) → "J.M. Boillot" (`266af94b`); "Vidal Fleury" (`9f9c976f`) → "Vidal-Fleury" (`58116bac`). Every reference to a duplicate is repointed, then the duplicate producer row is deleted.
      - The `catalog_wines_audit` trigger writes 5 `catalog_wine_edits` rows with `editor_id` null (the pooler session has no `auth.uid()`). Rows that point at the corrected wines by id (5 cellar lots, 4 WSET notes, 1 cellar consumption, 1 answer key) show the new identity without being written.
      - **Not approved, not changed:** L'Envolée (`149ce313`, producer still unknown); any designation, and any wine-name change beyond #5's; the duplicate "Tridente" row `0d1d099c`; Ampelos's wine name "Syrah"; the Madeira "Borges" row `25612e68`; the "Sangiovese blend" grape row. Producers left with no wines ("Vecchie Vigne Paitin", "Gasleni Alberti", "Tridente") are not deleted.
      - **#3's wine name needs the owner's check before any wine-name write.** The approval keeps "Vecchie Vigne" in the name, but the current "Sori Paitin Barbaresco Riserva" does not contain the words, and the approval does not say where they go. The label reads them first; L1b proposed "Sorì Paitin Vecchie Vigne". The script on disk and its last dry run leave the name unchanged.
      - **Left for its own owner decision:** glass `e73e39e5` in CLOSED tasting `295dc289` (revealed, 2 scored guesses) is linked to #1, and its answer key keeps Saint-Émilion AOC (`151dc287`). Changing it would alter scored history.
      - **After the live run:** `src/lib/wine-identity/__fixtures__/reference-snapshot.json` and `live-replay.test.ts` still hold the pre-fix rows (both J. M. Boillot duplicates, the bundled Borges name, Vidal Fleury). The tests keep passing on the fixtures. L1 re-exports the snapshot with `.superpowers/export-reference-snapshot.mjs`, which now exports `has_wines`. The gitignored `label-test-set.json` already expects the corrected catalog.
    - **Re-reads (approval 1; pending).** The main session re-reads the photos of #2, #4, #7, #12 and #15, once each, after 11d4c3f, with the L1 harness's round 2:
      `LABEL_LIVE=1 LABEL_REREAD=j-m-boillot-2024,changyu-moser-xv-2022,el-enemigo-2019,borges-2004,tridente-2018 npx vitest run --config .superpowers/add-wine-v2/vitest.live.config.mts`
      - It writes only `.superpowers/add-wine-v2/l1/r2/`, and refuses while the reader's instructions are still round 1's. `LABEL_REREAD_PREFLIGHT=1` with `LABEL_LIVE` unset rehearses it read-only.
      - About $0.11. L1's 15 reads cost $0.33; the five bring `label_reads` to **20 of D1's 30**, the approval's cap for this round. V3 still needs its one read (21). No second re-read round fits the approval.
      - G1's live-read gate becomes "at most 20 before V3". V3's check stays "below 30".
      - **#12 needs an owner decision before its re-read.** Under the brand rule a re-read will probably return "Borges". That name folds only to the Madeira row `25612e68`, and `find_producer_by_folded_name` does not filter by region (a Porto region only reorders ties). The draft would silently take the wrong house while the test set expects "Sociedade dos Vinhos Borges". Options include a region-scoped match, an alias, or relating the row to "Borges & Irmao" (`8830840e`).
      - Each stored re-read becomes a live fixture with its replay assertion, as in L1.
    - **CLAUDE.md (G4).** G4 rewrites CLAUDE.md's scanner line to state the five approved field rules and the designation short-form rule. It rewrites the producer-lookup line to state `find_producer_by_folded_name`'s order: region, exact spelling, holds wines, region link, name and id. The approved merges are data cleanup, not a rule.
...
23. **Late replies and the note hand-off (S5a, wave C3 review, 2026-09-13).** Four review rounds each found another server reply that acted on a screen the user had already left. This amendment replaces per-path guards with one rule. It supersedes S5a's and S5b's task text where they differ.
    - **A stale reply never acts.** `SheetState` gains `flow: number` (accepted, beyond spec §C.4), which increments on every user step that moves or closes the sheet: navigation, ←, opening by hand, the lot step or the chooser, a photo taking the screen, `requestClose`, `cancelClose` and Discard. Background steps (uploads, reads, failures, position and row updates) leave it unchanged. Every action that waits on the server carries the ticket it started with: `{ flow, itemId, view }`. One pure exported helper, `replyIsCurrent(state, ticket)`, decides. A stale reply still records its outcome (the bottle is marked added, or its row keeps the error, so nothing is lost), but it never navigates, closes the sheet, raises or ends a close-ask, or opens the note. A stale `notePicked` does nothing: the catalog write already happened, and the user can pick again. `catalogThenLot`'s guard and every other still-here check in the hook move into the reducer through this helper.
    - **A note pick asks before dropping work** (accepted deviation from spec §C.2's "the sheet closes and opens NewNoteModal"). `notePicked` closes and opens the note only when `shouldCloseAfterSingleAdd` would allow it. Otherwise it raises the close-ask holding the pick (`closeAsk.note`, accepted). S5b's footer contract: Discard calls `adds.discardAndClose()`, which hands the note on; Keep going calls `adds.send({ type: "cancelClose" })`.
    - **Ending a note ask keeps the pick while its own chain is on screen.** The pick is given back only when the confirm, chooser or form it was made on is gone (the `leavesAdoption` rule), never just because the ask ended.
    - **Tests.** The seeded model-based test gains late replies: a reply issued at one step is delivered at a random later step. It asserts: a stale reply never navigates, closes the sheet or changes a close-ask; every reply's outcome is recorded; the sheet never closes over unfinished work or a dirty form; a pick never outlives its chain. Each reviewer probe from wave C3 (A, A4, F, G, B3, B4) becomes a named test first.
    - **Review calibration for the S chain.** Medium means a wrong write, lost work, the sheet closing over unfinished work or a dirty form, or a broken binding rule. A rendering or copy detail for a later task is a hand-off, and polish is low.
24. **Owner approvals after round 2 (2026-09-13).** The owner answered the round-2 follow-ups with "Go with your recommendations". Add-wine ledger D19. This supersedes amendment 21 where they differ: its "not approved" list (the duplicate "Tridente" row `0d1d099c`, and keeping the producers left with no wines), its open #12 decision, and the old B.5 step 7 / B.7 / E.2 rules. The spec now states the new rules (§B.5 steps 4.10, 5, 6 and 7, §B.7, §E.2). Both code commits below are on master and not pushed.
    - **Alternative producer names (approvals 1 and 2, the alias half; committed, live apply pending).** Commit b505cbf adds `supabase/migrations/20260914113500_producer_aliases.sql`, `src/lib/wine-identity/producer-aliases.test.ts` (20 tests, written first) and the fallback in `__fixtures__/snapshot-lookup.ts`. The dry run printed `DRY-OK 20260914113500 producer_aliases`. Like 112500, the file is in no task's OWNS.
      - `public.producer_aliases` holds one curated row per alternative name: the producer it names (on delete cascade), the alias as written, and `alias_folded = f_search_norm(alias)` under a unique index, so a folded name names at most one producer. Access mirrors `producers` with no write: row level security on, the same select policy, SELECT only for anon, authenticated and service_role. Rows change only through migrations.
      - `find_producer_by_folded_name` keeps 20260914112500's producer query word for word, and falls back to an alias's producer only when no producer row folds equal. A producer row always beats an alias, and the region never filters or orders the alias half. The signature, SECURITY INVOKER, `search_path` and grants stay. `find_or_create_producer` is not recreated; it takes the fallback at run time, so a pending aliased name reuses that producer.
      - Exactly two seeds: "Borges Porto" → Sociedade dos Vinhos Borges (`6aaef358`), and "Tridente" → Bodegas Tridente (`7f46bd24`). There is no plain "Borges" alias: that name is the Madeira house (`25612e68`), and the migration asserts that no alias folds to "Borges" or names that house.
      - `ReferenceSnapshot` gains optional `aliases` (absent means none). Two folded-equal aliases, or an alias whose producer the snapshot lacks, throw as a broken snapshot.
      - The gitignored `.superpowers/export-reference-snapshot.mjs` now exports every alias with its producer, and refuses to run until the migration is live. The read-only probes and the dry-run log are in `.superpowers/add-wine-v2/probes/113500-*`.
      - **Pending, main session only.** Repeat amendment 11's version check (`select 1 from supabase_migrations.schema_migrations where version = '20260914113500'` must return no row), then run `node --env-file=.env.local scripts/scratch-apply.mjs --file supabase/migrations/20260914113500_producer_aliases.sql --mode live`. It can run before or after the merge script below: its checks accept `0d1d099c` present or already merged away. While the duplicate exists, its row wins "Tridente"; once the merge deletes it, the alias answers.
    - **Producer merges and deletions (approval 2's merge half, 5 and 6; main-session data change, live run pending).** One transaction in the gitignored `.superpowers/add-wine-v2/l1/catalog-fixes-2.mjs`, modelled on round 1's reviewed `catalog-fixes.mjs`; no migration. Step 1 resolves every row by full id and current value, and fails closed on any drift, unexpected reference, unreviewed trigger or unique collision. The dry run rolled back, the rollback was re-verified in a fresh read-only transaction, and every self-test scenario (A–E) behaved as expected.
      - **Tridente (approval 2).** The empty "Tridente" (`0d1d099c`, Castilla La Mancha, no catalog wines since amendment 21's catalog fix) merges into Bodegas Tridente (`7f46bd24`): every reference is repointed, then `0d1d099c` is deleted.
      - **Lascombes (approval 5).** The empty, region-less "Château Lascombes" copy (`80f05804`) merges into "Chateau Lascombes" (`43c67107`, Bordeaux), which holds the Margaux 2017 (`bb0a9d3c`) and a closed-tasting answer key. The copy is deleted, then the survivor is renamed in place, id kept, to "Château Lascombes" once that name is free.
      - **Orphaned misreads (approval 6).** "Vecchie Vigne Paitin" (`38a85744`) and "Gasleni Alberti" (`6b90fa8b`) are deleted after asserting that nothing references them.
      - What the dry run would write: 4 `producers` rows deleted and 1 renamed in place. No reference row is repointed, because none points at a merge duplicate. Every `catalog_wines` and `wine_answers` row is asserted byte-identical.
      - **Pending, main session only.** `node --env-file=.env.local .superpowers/add-wine-v2/l1/catalog-fixes-2.mjs --mode live`. It commits, then re-verifies in a fresh read-only transaction. Live mode never runs the self-test.
      - **Run the script and the alias migration back to back.** If the script runs first, no producer folds to "tridente" and no alternative name answers it until the alias is live, so a save through `find_or_create_producer('Tridente', …)` would recreate the duplicate (the dry run's LOOKUP GAP notice). Both orders should pass, by static analysis only, not proven:
        - the migration's checks accept `0d1d099c` present or already merged away;
        - with the aliases live first, the script discovers `producer_aliases.producer_id` (FK, on delete cascade) as a reference column. Both seeded aliases point at the survivors `7f46bd24` and `6aaef358`, which are never deleted. That table has no trigger and no other column named like `%producer%`, and the script's "Tridente" lookup check accepts null or `7f46bd24`.
      - Dry mode's self-test writes to live tables inside transactions that always roll back: one `producers` row (an update and an insert), one `wine_designation_members` row, and catalog wine `c4e1429e`, whose update fires the `catalog_wines_audit` trigger inside the rolled-back transaction. Each holds row locks for under a second, and the rollback is re-verified.
    - **Region conflict (approval 3; done, spec §B.5 step 7).** Commit e9909cd, in `resolve.ts`, with tests in `resolve.test.ts`. A read with no appellation text that is not a no-geographic-indication read takes its region from the read's region field alone. When the existing producer it resolved to has a region link in another region of the same country, the region is left empty for the user (no provenance, listed in `missing`), and step 7 does not refill it from the link. A blank is safer than a wrong answer that looks right.
      - A read with appellation text keeps its region, a no-GI read keeps its country's tier, a link in another country changes nothing, and an empty region is still filled from the link as before.
      - Step 7 now calls `regionById` in one new case: a region read alone that differs from the producer's link. On the server that is one memoized point read; every other path makes the same lookups as before.
    - **Ningxia synonym (approval 4; done, spec §B.5 step 4.10).** Also e9909cd, in `src/lib/label-scan/region-canonical.ts` (`APPELLATION_SYNONYMS`, `curatedAppellationName`) and `resolve.ts`. It is a curated table, not a heuristic.
      - Keys: the canonical country, then the stored region name, then the read's appellation text trimmed, lowercased and whitespace-collapsed. Values: the exact stored appellation name. Only own keys are read, so text such as "constructor" never reaches `Object.prototype`.
      - Three spellings map to "Ningxia" (`328c774b`) in region Ningxia, China: "Ningxia Helan Mountain Eastern Foothills", "Ningxia Helan Mountain East" and "Helan Mountain Eastern Foothills".
      - The synonym is tried only when no reference row agreed with the read's text, and only inside the region the read itself named. Its target goes through the same appellation steps and must be the one row in that region.
      - Deviation: `resolveAppellation`, an internal helper with no other callers in `src`, `scripts` or `.superpowers`, now returns `{ row, agreed }` instead of `row | null`, so rows that agreed but tied or were filtered out never fall through to the synonym.
    - **Replay pins (L1's `live-replay.test.ts`, in e9909cd).**
      - #4 round 2 is re-pinned on purpose: appellation Ningxia (`328c774b`), `missing: []`. It no longer equals its live draft.
      - Two new tests: #4 round 2's synonym evidence, and a #15 stand-in for approval 3. In the stand-in, the Tridente row carries Bodegas Tridente's region link; the read's Castilla-La Mancha is left blank, and `missing` is vintage, region and appellation.
      - #4 round 1 and #15 in both rounds keep their outcomes. Their `why` notes record why neither rule moves them under today's snapshot: #15's "Tridente" still finds `0d1d099c`, whose link is the read's own region. The partial-read test is narrowed to reads with no appellation text.
    - **Pending after both live applies: the snapshot refresh (L1).** `src/lib/wine-identity/__fixtures__/reference-snapshot.json` still holds producers `0d1d099c`, `38a85744` and `6b90fa8b`, and `live-replay.test.ts` and `resolve.test.ts` also mention them. As after round 1, L1 re-exports the snapshot with `node --env-file=.env.local .superpowers/export-reference-snapshot.mjs` and re-pins the replay. #15's `why` notes name the expected change in both rounds: once "Tridente" reaches Bodegas Tridente (Castilla y Leon), the regions disagree and the region is left blank.
    - **Left as they are (not approved).**
      - Catalog wine `bb0a9d3c` keeps wine_name "Chateau Lascombes" (unaccented), which repeats the producer name once the producer reads "Château Lascombes".
      - The revealed answer key of glass `3bc838b8` in CLOSED tasting `42e8c830` "Hornbæk 2026 FREDAG" keeps its producer id and will display "Château Lascombes". Only the accent changes, and scoring compares ids. The approval names this row.
      - Audit history keeps the ids of the deleted producers: `catalog_wine_edits` `030ad66e` and `680daba7` (Tridente), `6dfdc714` (Vecchie Vigne Paitin) and `ea172c5a` (Gasleni Alberti). No code under `src` reads `catalog_wine_edits`, so no page resolves those ids.
    - **Items 7 and 8: left unchanged by decision.** "Domaine Laroche" (`99bff83a`) and "Domaine La Roche" (`f62327c0`) stay two producers; the merge script asserts both and writes neither. The closed-tasting answer key for Château Saint Georges 2012 (glass `e73e39e5`, CLOSED tasting `295dc289`) keeps Saint-Émilion AOC (`151dc287`), as amendment 21 left it.
    - **CLAUDE.md (G4).** G4 adds the alternative-name fallback to CLAUDE.md's producer-lookup line, and the region-conflict and curated-synonym rules to its scanner line. The merges and deletions are data cleanup, not a rule.
