...
/**
 * The semi-blind reveal (SB4; spec §10.3 item 3, §7.3 item 6). Renders
 ...
 * was …" row for it. Never the blind flow's per-category answer card: this
 * is the only surface a semi-blind reveal ever gets.
 *
      ...
      <div className="flex flex-col gap-[14px] px-4 pb-4 md:flex-row md:items-start md:gap-6">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-[14px]">
          <div className="flex flex-col items-center gap-[9px] pt-[10px] pb-1 text-center">
            ...
            <span className="font-heading text-[38px] font-semibold leading-none text-gold-dark md:text-[46px]">
              {identity.producer} {identity.vintage}
            </span>
            {identity.meta ? (
              <span className="text-[12.5px] text-muted-foreground">{identity.meta}</span>
            ) : null}
          </div>
          ...
          {result && resultCopy ? (
            ...
            >
              {result.hit ? (
                <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-gold text-on-accent">
              ...
              <span className="flex min-w-0 flex-1 flex-col gap-0.5">
                ...
                <span
                  className={cn(
                    ...
                    result.hit ? "text-gold-dark" : "text-muted-foreground",
                  )}
                >
                  {resultCopy.detail}
                </span>
            ...
            <div className="flex items-center gap-[10px] rounded-[13px] border border-border-light bg-card p-[14px_16px]">
        ...
        <div className="flex shrink-0 flex-col gap-[16px] md:w-[300px]">
                    ...
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span
                        className={cn(
                          ...
                          s.correct ? "text-gold-dark" : "text-rose",
                        )}
                      >
                        {s.count}
                      </span>
          ...
          {poolLines.length > 0 ? (
            <div className="flex flex-col gap-1">
              {poolLines.map((line) => (
                <p key={line} className="text-[11.5px] text-muted-foreground">
                  {line}
                </p>
              ))}
            </div>
                ...
                {standings.map((s, i) => (
                  <div
                    ...
                    className={cn(
                      ...
                      i < standings.length - 1 && "border-b border-border-light",
                    )}
                  >
                    <span
                      className={cn(
                        "w-6 shrink-0 font-heading text-[15px] lining-nums tabular-nums",
                        s.rank === 1 ? "text-gold-dark" : "text-muted-foreground",
                      )}
                    >
                      {rankLabel({ rank: s.rank, tied: s.tied })}
                    </span>
                    <span className="flex-1 truncate text-[13.5px]">{s.name}</span>
                    <span className="text-[13.5px] tabular-nums text-gold-dark">
                      {s.matches}
                    </span>
                  </div>
                ))}
