      ...
      <div className="flex w-full max-w-[1500px] flex-1 flex-col gap-6 p-6 sm:p-8">
        ...
        <div className="flex gap-8">
          {/* Side nav: search + a dense, scrollable jump list (icon + name) so
              you can skim the whole library and click straight to a card. The
              search sits here on desktop; below lg the nav is hidden and the
              search moves inline in the content column. */}
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-6">
            <div>
              ...
              <p className="mt-2 text-muted-foreground">
                The same grape list used across every tasting, with tasting
                notes for the most common varieties.
              </p>
            </div>

            {/* Mobile search (the side-nav search is hidden below lg); the
                colour filter stays inline at every width. */}
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <form method="GET" className="flex-1 lg:hidden">
                {color ? (
                  <input type="hidden" name="color" value={color} />
                ) : null}
                <Input
