...
// The Rules section as a panel for the Library tab shell (same content as the
    ...
    <div className="flex w-full max-w-2xl flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-muted-foreground">
          Fully-blind tastings on Blindr are scored with the Danish national
          championship point system. For each wine you earn points per category
          you guess correctly:
        </p>
      </div>
          ...
          <ul className="flex flex-col divide-y divide-border">
            {ROWS.map((r) => (
              ...
              >
                ...
                <p className="shrink-0 font-semibold tabular-nums">{r.points}</p>
              </li>
            ))}
          </ul>
          <p className="mt-4 text-sm text-muted-foreground">
            A wine with every category in play is worth up to{" "}
            <span className="font-semibold text-foreground">30 points</span>. A
            wine with no secondary grape, appellation, or type designation simply
            has fewer scorable categories.
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-2 text-sm text-muted-foreground">
          <p>
            Guess the{" "}
            <span className="font-medium text-foreground">exact year</span> for 2
            points, or land{" "}
            <span className="font-medium text-foreground">within one year</span>{" "}
            (e.g. 2019 when the answer is 2018 or 2020) for 1 point.
          </p>
          <p>
            Non-vintage (NV) and tawny (10 / 20 / 30 / 40+ years) are their own
            vintage types and only score on an exact match — no off-by-one credit.
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="text-sm text-muted-foreground">
          In a semi-blind tasting the full wine list is shown up front and you
          just match each glass to a wine — so there&apos;s no category
          breakdown. You simply score 1 point per glass matched to the correct
          wine (e.g. &ldquo;4/6 correct&rdquo;).
        </CardContent>
      </Card>
    </div>
  );
