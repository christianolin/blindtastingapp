...
// The current-boundary set moved out of this suite to
// scripts/wine-map-sources/boundary-expectations.test.mjs. It guards the MAP
// rather than the schema, and it was the only check here that touches nothing
// -- every statement is a select. This suite writes: mostly inside a rollback,
// but the concurrency test commits two wine_places rows and clears them in a
// finally, opens extra connections, switches role, and waits on an advisory
// lock. That is fine to run deliberately; it is not fine on every pull
// request, which is where the read-only half now runs.
