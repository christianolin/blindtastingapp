// LOCAL, READ-ONLY shape/provenance preview for the Beaujolais region + its
// appellations (data/wine-map/beaujolais-appellations.json). For each target it
// pages the INAO parcel layer (WFS LIKE bound), filters to exact membership,
// and runs the SAME client-side concave dissolve the live pipeline uses
// (concave-engine.mjs) - so the rendered outline is what WOULD be staged. Writes
// only a labelled SVG + numeric report to .superpowers/sdd/ (gitignored);
// touches NO database and NO storage, stages nothing. Exits non-zero if any
// target dissolves to nothing or spills outside the region window, so it doubles
// as a provenance/geometry gate for the owner's shape review.
//
// Usage: node scripts/wine-map-sources/preview-beaujolais.mjs [--only=slug,slug]
import { readFile, writeFile, mkdir } from "node:fs/promises";
import { wfsPageUrl, parcelMatches, PAGE_SIZE } from "./inao-lib.mjs";
import { buildConcaveGeometry } from "./concave-engine.mjs";

const ARTIFACT = "data/wine-map/beaujolais-appellations.json";
const OUT_DIR = ".superpowers/sdd";
const SVG_OUT = `${OUT_DIR}/preview-beaujolais.svg`;
...
console.log(`beaujolais preview: ${targets.length} target(s)${only ? ` (only ${only.join(",")})` : ""}`);
...
// --- SVG (shared canvas, region drawn faint under the crus) -----------------
...
for (const d of [...drawn].sort((a, b) => a.name.localeCompare(b.name))) {
  console.log(
    `  ${d.inside ? "ok " : "OUT"} ${d.name.padEnd(20)} ${String(d.level).padEnd(8)} parts ${String(d.components).padStart(2)}  vtx ${String(d.vertices).padStart(4)}  bbox ${d.bbox[0].toFixed(3)},${d.bbox[1].toFixed(3)}..${d.bbox[2].toFixed(3)},${d.bbox[3].toFixed(3)}`,
  );
}
...
console.log(`\nGATE OK: ${drawn.length} Beaujolais footprints dissolved and in-window.`);
