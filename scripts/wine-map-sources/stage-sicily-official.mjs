// Task: stage DRAFT wine_place_boundaries for the Sicily round-1 zones
...
// of stage-sicily-official.mjs.
//
// Source: data/wine-map/sicily-comuni-dissolved.geojson — a FeatureCollection
...
// Usage:
//   node scripts/wine-map-sources/stage-sicily-official.mjs           (test only, default)
//   node scripts/wine-map-sources/stage-sicily-official.mjs --stage   (controller-gated; persists)
...
const SOURCE_FILE = "data/wine-map/sicily-comuni-dissolved.geojson";
const NAMESPACE = "SICILY_COMUNI";
...
for (const boundary of BOUNDARIES) {
  ...
  const title = `Sicily — ${boundary.label} (DRAFT, official Regione Piemonte delimited area)`;
  const svg =
    ...
    `</svg>\n`;
  const outPath = `${OUT_DIR}/preview-sicily-${boundary.key}-official.svg`;
...
const importer = `scripts/wine-map-sources/stage-sicily-official.mjs@${
...
try {
  for (const boundary of BOUNDARIES) {
    ...
    const provenanceNote =
      `Comune-union footprint for "${boundary.name}" (ISTAT comuni per disciplinare) ` +
