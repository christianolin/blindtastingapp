// Task: stage DRAFT wine_place_boundaries for the Alto Adige round-1 zones
...
// of stage-altoadige-official.mjs.
//
// Source: data/wine-map/altoadige-doc-igt.geojson — a FeatureCollection
...
// Usage:
//   node scripts/wine-map-sources/stage-altoadige-official.mjs           (test only, default)
//   node scripts/wine-map-sources/stage-altoadige-official.mjs --stage   (controller-gated; persists)
...
const SOURCE_FILE = "data/wine-map/altoadige-doc-igt.geojson";
const NAMESPACE = "ALTOADIGE_DOC_IGT";
const AUTHORITY = "Provincia Autonoma di Bolzano — Ufficio Frutti-viticoltura";
const JURISDICTION = "Italy";
const LICENCE = "CC BY 4.0";
const SIMPLIFY_TOLERANCE = 0.0002;
// Alto Adige staging window: [minLon, minLat, maxLon, maxLat].
const WINDOW = { minLon: 10.5, minLat: 46.1, maxLon: 12.0, maxLat: 47.0 };
...
const BOUNDARIES = [
  { key: "alto-adige", targetKey: "italy.trentino-alto-adige.alto-adige", name: "Alto Adige", label: "Alto Adige" },
  { key: "santa-maddalena", targetKey: "italy.trentino-alto-adige.santa-maddalena", name: "Santa Maddalena", label: "Santa Maddalena" },
  { key: "terlano", targetKey: "italy.trentino-alto-adige.terlano", name: "Terlano", label: "Terlano" },
  { key: "meranese", targetKey: "italy.trentino-alto-adige.meranese", name: "Meranese", label: "Meranese" },
  { key: "valle-isarco", targetKey: "italy.trentino-alto-adige.valle-isarco", name: "Valle Isarco", label: "Valle Isarco" },
  { key: "val-venosta", targetKey: "italy.trentino-alto-adige.val-venosta", name: "Val Venosta", label: "Val Venosta" },
  { key: "colli-di-bolzano", targetKey: "italy.trentino-alto-adige.colli-di-bolzano", name: "Colli di Bolzano", label: "Colli di Bolzano" },
  { key: "lago-di-caldaro", targetKey: "italy.trentino-alto-adige.lago-di-caldaro", name: "Lago di Caldaro", label: "Lago di Caldaro" },
  { key: "valdadige", targetKey: "italy.trentino-alto-adige.valdadige", name: "Valdadige", label: "Valdadige" },
];
...
for (const boundary of BOUNDARIES) {
  ...
  const title = `Alto Adige — ${boundary.label} (DRAFT, official Regione Piemonte delimited area)`;
  const svg =
    ...
    `</svg>\n`;
  const outPath = `${OUT_DIR}/preview-altoadige-${boundary.key}-official.svg`;
...
const importer = `scripts/wine-map-sources/stage-altoadige-official.mjs@${
...
try {
  for (const boundary of BOUNDARIES) {
    ...
    const generation = {
      ...
      note:
        "Authoritative delimited zone from the Provincia Autonoma di Bolzano official " +
        "Zone DOC e IGT dataset (GeoKatalog) — interior rings (holes) are real and were not stripped.",
    };
    ...
    const provenanceNote =
      `Official Provincia di Bolzano DOC/IGT polygon(s) for "${boundary.name}" ` +
