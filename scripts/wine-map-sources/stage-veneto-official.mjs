// Task: stage DRAFT wine_place_boundaries for the Veneto round-1 zones
...
// of stage-veneto-official.mjs.
//
// Source: data/wine-map/veneto-doc-docg.geojson — a FeatureCollection
...
// fetch — this script reads that one committed file, matching by `name`.
//
...
// Usage:
//   node scripts/wine-map-sources/stage-veneto-official.mjs           (test only, default)
//   node scripts/wine-map-sources/stage-veneto-official.mjs --stage   (controller-gated; persists)
...
const SOURCE_FILE = "data/wine-map/veneto-doc-docg.geojson";
const NAMESPACE = "VENETO_DOC_DOCG";
const AUTHORITY = "Regione del Veneto — Servizio qualificazione delle produzioni agroalimentari";
const JURISDICTION = "Italy";
const LICENCE = "CC BY 4.0";
const SIMPLIFY_TOLERANCE = 0.0002;
// Veneto staging window: [minLon, minLat, maxLon, maxLat].
const WINDOW = { minLon: 10.5, minLat: 44.9, maxLon: 13.2, maxLat: 46.8 };
...
const BOUNDARIES = [
  { key: "valpolicella", targetKey: "italy.veneto.valpolicella", name: "Valpolicella", label: "Valpolicella" },
  { key: "soave", targetKey: "italy.veneto.soave", name: "Soave", label: "Soave" },
  { key: "bardolino", targetKey: "italy.veneto.bardolino", name: "Bardolino", label: "Bardolino" },
  { key: "prosecco", targetKey: "italy.veneto.prosecco", name: "Prosecco", label: "Prosecco" },
  { key: "conegliano-valdobbiadene-prosecco", targetKey: "italy.veneto.conegliano-valdobbiadene-prosecco", name: "Conegliano Valdobbiadene Prosecco", label: "Conegliano Valdobbiadene Prosecco" },
  { key: "lugana", targetKey: "italy.veneto.lugana", name: "Lugana", label: "Lugana" },
  { key: "bianco-di-custoza", targetKey: "italy.veneto.bianco-di-custoza", name: "Bianco di Custoza", label: "Bianco di Custoza" },
  { key: "colli-euganei", targetKey: "italy.veneto.colli-euganei", name: "Colli Euganei", label: "Colli Euganei" },
  { key: "colli-berici", targetKey: "italy.veneto.colli-berici", name: "Colli Berici", label: "Colli Berici" },
  { key: "breganze", targetKey: "italy.veneto.breganze", name: "Breganze", label: "Breganze" },
  { key: "gambellara", targetKey: "italy.veneto.gambellara", name: "Gambellara", label: "Gambellara" },
...
for (const boundary of BOUNDARIES) {
  ...
  const title = `Veneto — ${boundary.label} (DRAFT, official Regione Piemonte delimited area)`;
  const svg =
    ...
    `</svg>\n`;
  const outPath = `${OUT_DIR}/preview-veneto-${boundary.key}-official.svg`;
...
const importer = `scripts/wine-map-sources/stage-veneto-official.mjs@${
...
try {
  for (const boundary of BOUNDARIES) {
    ...
    const generation = {
      ...
      note:
        "Authoritative delimited zone from the Regione del Veneto official DOC/DOCG " +
        "viticultural zones dataset (IDT2) — interior rings (holes) are real and were not stripped.",
    };
    ...
    const provenanceNote =
      `Official Regione del Veneto DOC/DOCG polygon(s) for "${boundary.name}" ` +
