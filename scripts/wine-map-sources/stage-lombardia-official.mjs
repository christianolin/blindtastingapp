...
// of stage-lombardia-official.mjs.
//
// Source: data/wine-map/lombardia-comuni-dissolved.geojson — a FeatureCollection
...
// Usage:
//   node scripts/wine-map-sources/stage-lombardia-official.mjs           (test only, default)
//   node scripts/wine-map-sources/stage-lombardia-official.mjs --stage   (controller-gated; persists)
...
const SOURCE_FILE = "data/wine-map/lombardia-comuni-dissolved.geojson";
const NAMESPACE = "LOMBARDIA_COMUNI";
...
for (const boundary of BOUNDARIES) {
  ...
  const title = `Lombardy — ${boundary.label} (DRAFT, official Regione Piemonte delimited area)`;
  const svg =
    ...
    `</svg>\n`;
  const outPath = `${OUT_DIR}/preview-lombardia-${boundary.key}-official.svg`;
...
const importer = `scripts/wine-map-sources/stage-lombardia-official.mjs@${
