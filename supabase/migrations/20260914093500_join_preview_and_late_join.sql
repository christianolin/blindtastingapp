...
-- .superpowers/blind-tasting/probes/20260914093500-live-defs.sql), never an
...
--   DECLINED) to JOINED and keeps an existing joined_at.
...
--   (BEFORE UPDATE; lane N 20260912091000), and "participants update own or
--   host" lets a participant or the host write status and joined_at at any
--   time.
-- * The enum types are tasting_status, timing_mode and reveal_mode_type: the
--   names spec §4.4 uses.
-- M1-M3 (20260914090500, 20260914091500, 20260914092500) touch none of these
-- objects: this file applies on live as it is, and after them.
--
-- What this migration does:
-- 1. Spec §4.4 (between the two banners below), verbatim apart from the one
--    line in item 3: host_tastings_count and get_join_preview;
--    generate_join_code with its loop bound 6 -> 10 (the rest of the body is
--    the live one: the post-state block asserts its md5); the leave guard and
--    the joined_at stamp with their triggers; EXECUTE on both trigger functions
...
--    written; the new trigger replaces the value. CREATE OR REPLACE keeps its
--    ACL, SECURITY DEFINER, search_path and volatility (asserted).
-- 3. One line differs from spec §4.4's block (review fix, 2026-09-13). The
--    leave guard's second test reads "auth.uid() is not null" where the spec
--    has "auth.uid() = new.user_id". Spec §4.4 and §5.4 say neither a
--    participant nor the host can move joined_at, but the spec's line bound
--    only the participant: after Start the host could set a guest JOINED ->
--    DECLINED -> JOINED, and the stamp would give that guest a fresh joined_at,
--    turning every glass already revealed into "joined after" (0 points). Now
--    no signed-in caller, the host included, moves a row out of JOINED after
--    Start; service_role (auth.uid() null) stays free. Spec §4.4's SQL block
--    needs the same one-line amendment.
--
...
-- * The leave guard stops a JOINED guest lowering reveal_wine's eligible count
--   after Start, and the host's own row never leaves JOINED, whoever writes it
--   (service_role included). After Start it refuses any signed-in caller, the
--   host too, moving a row out of JOINED (item 3); before Start the host can
--   still take a guest off the list. It reads new.tasting_id and new.user_id,
--   which is sound only because tasting_participants_pin_identity refuses
--   changing either: a combined status + tasting_id update is still refused.
--   BEFORE triggers fire in name order: leave_guard, pin_identity,
--   stamp_joined_at.
-- * joined_at is trigger-owned: a client-sent value is replaced on every insert
--   and update, and after Start no signed-in caller can take a row out of
--   JOINED and back, so neither a participant nor the host can move it to turn
--   glasses into, or out of, "joined after" (§5, §11). Still possible, and
--   bounded: the host can delete a participant row ("participants delete
--   host"), which deletes that guest's guesses with it (ON DELETE CASCADE), so
--   a rejoin starts a new row with nothing scored; service_role is unbound.
...
--
-- Deployed code once applied (after add-wine V2, spec §1.5 and §15):
...
--   refused.
-- * respondToInvite, createTasting's host row, inviteToTasting and
--   scripts/seed-demo-people.mjs keep working; any joined_at they send is
--   replaced by now().
...
--   the error.
-- * No deployed or planned flow moves another participant out of JOINED: the
--   host's participant writes are inserts (createTasting, inviteToTasting), and
--   the coming leaveTasting and transfer_tasting_host run in DRAFT only. The
--   host refusal after Start (item 3) refuses nothing the app does.
  ...
  where a.attnum > 0 and not a.attisdropped
    and ((a.attrelid = 'public.tastings'::regclass
          and a.attname in ('id', 'name', 'host_id', 'scheduled_at', 'reveal_mode', 'timing_mode',
                            'sequential_guessing', 'status', 'join_code'))
...
-- ---------------------------------------------------------------------------
-- Spec §4.4, verbatim apart from one line of the leave guard (header item 3).
-- ---------------------------------------------------------------------------
...
begin
  if old.status = 'JOINED' and new.status <> 'JOINED' then
    ...
    if auth.uid() is not null and v_status <> 'DRAFT' then
...
begin
  ...
  for v_fn in
    ...
    from (values
      ...
      ('public.tasting_participants_leave_guard()', true, '{search_path=public}', 'v', 'plpgsql', 'trigger', false,
       '', '1a6bee7c9ad8b832c64372e01f93a415', 'OWNER,service_role'),
      ('public.tasting_participants_stamp_joined_at()', false, '{search_path=public}', 'v', 'plpgsql', 'trigger', false,
       '', '552a2fb98280227b987cc3d59586d3ba', 'OWNER,service_role')
  ...
  --    The leave guard: the one line that differs from spec §4.4 (header item 3).
  ...
  if (select strpos(replace(p.prosrc, chr(13), ''), '    if auth.uid() is not null and v_status <> ''DRAFT'' then' || chr(10))
      from pg_proc p where p.oid = to_regprocedure('public.tasting_participants_leave_guard()')) = 0 then
    raise exception 'the leave guard does not refuse every signed-in caller after Start';
  end if;
  ...
  --    guard BEFORE UPDATE OF status, the lane N pin BEFORE UPDATE, the stamp
  --    BEFORE INSERT OR UPDATE; all row-level (tgtype ROW 1 | BEFORE 2 | INSERT 4
  --    | UPDATE 16), enabled, without WHEN.
  ...
  where t.tgrelid = 'public.tasting_participants'::regclass and not t.tgisinternal;
  if v_text is distinct from
       'tasting_participants_leave_guard 19 O status tasting_participants_leave_guard; '
  ...
  if (select md5(p.prosrc) from pg_proc p where p.oid = to_regprocedure('public.ensure_join_code(uuid)'))
     ...
     or (select md5(p.prosrc) from pg_proc p where p.oid = to_regprocedure('public.pin_tasting_participant_identity()'))
       is distinct from '32f3c8c10f6106303dcdc3ff5c062cca' then
    raise exception 'ensure_join_code or pin_tasting_participant_identity changed post-migration';
  end if;
