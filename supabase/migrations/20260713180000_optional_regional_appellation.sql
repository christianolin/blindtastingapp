-- Many real wines carry no appellation more specific than the region itself
-- (a plain "Bourgogne rouge", "Alsace", generic "Rioja", "California", a
-- Mendoza varietal) or no formal appellation at all (e.g. "Vin de France").
-- The LWIN import's region-level fallback only ran for regions that had
-- *zero* appellation candidates, so 181 of 378 regions (including Bordeaux,
-- Rhône, Alsace, Rioja, California, Mendoza) had specific sub-appellations
-- but no option matching the region's own name. This adds the missing ones,
-- and makes appellation itself optional — like secondary grape and type
-- designation, it's only scored when the wine's answer key actually has one.

insert into appellations (region_id, name)
select r.id, r.name
from regions r
where not exists (
  select 1 from appellations a where a.region_id = r.id and lower(a.name) = lower(r.name)
);

alter table wine_answers alter column appellation_id drop not null;
