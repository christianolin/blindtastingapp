-- Vallee du Rhone — meridional outline re-derive (revision flip).
--
-- france.rhone.meridional was derived from its 8 original crus; with
-- Vacqueyras now verified as its 9th child, derive-boundary.mjs re-derived it
-- from all 9 (362 vtx, 4 components — the commune-union blob joins the east
-- cluster). This revision flip atomically retires the old 8-cru current
-- boundary and promotes the 9-cru outline. Window = the meridional window
...
begin
  ...
  if v_place is null then
    raise exception 'france.rhone.meridional place missing';
  end if;

  select count(*) into v_count from wine_place_boundaries where wine_place_id = v_place and is_current;
  if v_count <> 1 then
    raise exception 'expected exactly 1 current meridional boundary pre-flip, got %', v_count;
  end if;
  select count(*) into v_count from wine_place_boundaries where wine_place_id = v_place and quality_status = 'DRAFT';
  if v_count <> 1 then
    raise exception 'expected exactly 1 DRAFT meridional boundary pre-flip, got %', v_count;
  end if;
  ...
  if v_minx < 4.5 or v_miny < 43.9 or v_maxx > 5.2 or v_maxy > 44.5 then
    raise exception 'meridional new boundary %,%,%,% escapes the window',
  ...
  select count(*) into v_count from wine_place_boundaries where wine_place_id = v_place and is_current;
  if v_count <> 1 then
    raise exception 'meridional must have exactly 1 current boundary post-flip, got %', v_count;
  end if;
  if not exists (
    select 1 from wine_place_boundaries where id = v_new and is_current and quality_status = 'VALIDATED'
  ) then
    raise exception 'new meridional boundary not current/validated';
  end if;
