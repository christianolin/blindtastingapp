-- Producers aren't a browsable entity (no producer page), so drop them as a
-- standalone search result. Typing a producer name still surfaces that
-- producer's WINES — the wine branch already matches on producer name.
