-- save_wset_note gains blind/training context: context_kind + tasting_wine_id.
-- On insert they come from the payload (default OPEN); on update they are
-- coalesced so editing a note never resets the context it was created with.
...
begin
  if exists (
    ...
    insert into wset_notes (
      id, catalog_wine_id, context_kind, tasting_wine_id, author_id, tasted_on,
...
do $$
begin
  if not exists (select 1 from pg_proc where proname = 'save_wset_note') then
    raise exception 'final-state: save_wset_note missing';
  end if;
end $$;
